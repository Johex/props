module.exports = {
    ...
    moveOrDelete: function moveOrDelete(todoToUse, move, remove, finishOrUndo, currentStatus, archive, unarchive) {
        ...
        if (move == true && finishOrUndo == 1){
            query = "UPDATE todo SET done = " + finishOrUndo + ", dateFinished = CURRENT_TIMESTAMP()  WHERE ID = '";
            console.log('moving...TOFINISHED..........');
        }

        //must item be moved back to to-do
        else if (move == true && finishOrUndo == 0){
            query = "UPDATE todo SET done = " + finishOrUndo + ", dateFinished = NULL  WHERE ID = '";
        ...
        //Must item be archived?
            ...
            query = "UPDATE todo SET archived = 1, dateArchived = CURRENT_TIMESTAMP()  WHERE ID = '";
        }

        //must item be unarchived?
        else if (unarchive = true){
            query = "UPDATE todo SET archived = 0, dateArchived = NULL WHERE ID = '"
        }
